...
### [2026-03-19] 기능만 확인하고 제약/한계 미검증 시 잘못된 권장 발생
- **상황**: Agent Teams 리서치 시 기능(features) 설명만으로 "가장 유망"이라고 권장했으나, 공식 문서를 직접 확인하니 모든 팀원이 동일 프로젝트 context를 공유하여 사용자 유스케이스(PE Expert ↔ 생성된 전문가 간 대화)와 근본적으로 충돌
- **근본 원인**: 기존 핵심 규칙 1번(근거 기반)은 "출처 제시"에 초점이었고, 규칙 2번(불확실성 명시)은 "잘못된 확신"에는 트리거되지 않음. 출처는 있었으나 개요 수준이었고, 사용자의 핵심 요구사항을 실제로 충족하는지 검증하는 의무가 없었음
- **해결**: CLAUDE.md + claude-md-template.md 핵심 규칙 1번에 적합성 검증 bullet 추가: "솔루션/도구를 권장할 때는 기능뿐 아니라 제약/한계도 공식 문서에서 직접 확인. 개요 수준의 정보만으로 단정적 권장 금지"
- **교훈**: 출처가 있어도 깊이가 부족하면 잘못된 확신으로 이어진다. 기능 확인은 필요조건이지 충분조건이 아니다 — 제약/한계 검증이 동반되어야 비로소 적합성 판단이 가능하다
...
### [2026-03-16] 세션 재시작 전 미완료 작업 미기록
- **상황**: youtube-data MCP 서버 추가 후 세션 재시작이 필요했고, 사용자가 테스트를 요청한 상태에서 세션 종료
- **문제**: 새 세션 시작 시 MEMORY.md에 미완료 작업 기록이 없어, 테스트 요청을 인지하지 못함
- **해결**: MEMORY.md에 `## 다음 세션 할 일` 섹션 신설, 핵심 규칙 3번에 세션 핸드오프 절차 추가, memory-system-guide.md에 세션 핸드오프 가이드 추가
- **교훈**: 세션 재시작/종료가 필요할 때는 반드시 미완료 작업을 MEMORY.md에 기록해야 한다. 기록 없이 종료하면 새 세션에서 맥락이 완전히 단절된다
