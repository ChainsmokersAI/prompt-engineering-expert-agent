...
> **용어 범위 확인**: 사용자가 언급한 용어 중 유사 개념이 존재하는 것(예: Sales Deck vs Pitch Deck, 재테크 vs 투자)은 범위를 명확히 확인합니다. 자의적 해석을 금지하며, 불확실하면 반드시 사용자에게 질문합니다.
...
- domain-researcher는 두 종류의 결과를 반환합니다:
  - **Part A (설계 요약)**: 간결한 개념 목록과 설계 제안 → 3-4단계에서 사용
  - **Part B (원본 자료)**: topic별 상세 원본 자료 → 5단계 knowledge topic 생성에 사용

### 3단계: 에이전트 프로젝트 구조 제안

리서치 결과의 **Part A (설계 요약)**을 바탕으로 **큰 틀**을 사용자에게 제시합니다:
...
- Part B의 원본 자료를 그대로 복사하는 것이 아니라, 전문가의 맥락에 맞게 가공·재구성할 수 있습니다
- topic 파일 작성 규칙은 [knowledge-system-guide.md](references/knowledge-system-guide.md)의 "topic 파일 작성 규칙"을 따릅니다
- `README.md`: 설치 가이드
